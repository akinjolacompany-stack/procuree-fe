...
export default function AcceptInviteClient({
  ...
  const invitePhone = normalizePhoneNumber(phone ?? "");
  const inviteToken = (token ?? "").trim();
  const hasInviteReference = Boolean(inviteToken || inviteEmail || invitePhone);
  ...
  async function handleSubmit(
    ...
    { setStatus, setSubmitting }: FormikHelpers<AcceptInviteFormValues>,
  ) {
    setStatus(undefined);
          ...
          {({ isSubmitting, status, setFieldValue }) => (
            <Form className="space-y-4">
              ...
              <FormikInput
                ...
                autoComplete="tel"
                onChange={(event) => {
                  setFieldValue("phone", normalizePhoneNumber(event.target.value));
                }}
              />
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={!hasInviteReference || isSubmitting || acceptInviteSignupMutation.isPending}
              >
                {isSubmitting || acceptInviteSignupMutation.isPending ? "Creating account..." : "Create Account"}
              </Button>

              <p className="text-center text-sm text-slate-400">
                Already have an account? {" "}
                <Link href="/signin" className="font-semibold uppercase text-sky-600 hover:underline">
                  Login Here
                </Link>
              </p>
            </Form>
